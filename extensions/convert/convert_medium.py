...
#https://github.com/Radu-Raicea/Interesting-People-On-Medium
...
def convert(source, crossrefQuery=''):
    ...
    #interesting_users = get_interesting_users('Radu_Raicea', min_recommendations)
    #print(interesting_users)

    username = source[source.find('@') + 1 :]

    user_id = get_user_id(username)

    print '-----posts------'
    for post in get_list_of_latest_posts([username]):

        print '| ' + post['title'].encode('utf-8') + ' | ' + MEDIUM + '/@' + username + '/' + post['uniqueSlug'].encode('utf-8') + ' | '
    
    print '-----following------'

    users = get_list_of_followings_user(user_id)

    for user in users:
        desc = ''
        if user.has_key('facebookAccountId') and user['facebookAccountId'] != '':
            desc += 'facebook:' + user['facebookAccountId'] + ' '
        if user.has_key('twitterScreenName') and user['twitterScreenName'] != '':
            desc += 'twitter:' + user['twitterScreenName'] + ' '
        desc += 'icon:' + "https://cdn-images-1.medium.com/fit/c/60/60/" + user['imageId'] + ' '

        desc += 'description:' + user['bio']

        line = ' | ' + user['name'] + ' | ' + MEDIUM + '/@' + user['username'] + ' | ' + desc

        print line.encode('utf-8')


    '''
    print '-----posts of following-----'
    count = 0
    for user in usernames:
        count += 1
        posts = get_list_of_latest_posts([user])

        for post in posts:
            line = ' | ' + post['title'] + ' | ' + MEDIUM + '/@' + user + '/' + post['uniqueSlug'] + ' | medium:@' + user
            print line.encode('utf-8')
...
def get_list_of_followings_user(user_id):
